...
import network.tos.blockchain.ton.extensions.isValidTonDomain
...
import network.tos.extensions.hasUnsupportedQuery
import network.tos.extensions.hostOrNull
import network.tos.extensions.pathOrNull
import network.tos.extensions.query
import network.tos.extensions.queryBoolean
import network.tos.extensions.queryLong
import network.tos.extensions.queryPositiveLong
...
sealed class DeepLinkRoute {
    ...
    sealed class Tabs(val tabUri: String, open val from: String): DeepLinkRoute() {
        ...
        ): Tabs("tos://wallet", from)

        data class Activity(
            override val from: String
        ): Tabs("tos://activity", from)
        ...
        data class Browser(
            ...
            private companion object {
                ...
                private fun buildBrowserUri(category: String?): String {
                    val builder = "tos://browser".toUri().buildUpon()
    ...
    data class DApp(val url: String): DeepLinkRoute() {
        ...
        private companion object {
            ...
            private fun parseLink(uri: Uri): String {
                var path = uri.toString().replace("tos://dapp/", "").toUriOrNull()
                if (path == null) {
