...
import network.tos.extensions.asJSON
import network.tos.wallet.data.rn.expo.encryptors.AESEncryptor
import network.tos.wallet.data.rn.expo.encryptors.HybridAESEncryptor
import network.tos.wallet.data.rn.expo.encryptors.KeyBasedEncryptor
...
internal class SecureStoreModule(
    private val context: Context
) {
    private val mAESEncryptor = AESEncryptor()

    private val keyStore = loadKeystore()
    private val hybridAESEncryptor = HybridAESEncryptor(context, mAESEncryptor)
    private val authenticationHelper = AuthenticationHelper()

    fun setActivity(activity: FragmentActivity) {
        authenticationHelper.activity = activity
    }
    ...
    suspend fun getItemImpl(key: String, options: SecureStoreOptions = SecureStoreOptions()): String? {
